locals {
  ...
  ########################## CODEPIPELINE
  codepipeline_name                     = local.ecs_destroy_trigger_flag == true ? null : "${var.env}-${local.ecs_service_name}-${var.cpl_name}"
  codepipeline_source_connection_name   = format("%s-github-connection", var.cpl_name)
  codepipeline_s3_artifact_name         = var.s3_bucket_name
  suorce_provider                       = var.suorce_provider
  cpl_tags                              = merge(var.cpl_tags, { Owner = var.owner, Environment = var.env })
  # CodeStar에 Github이 없을 경우
  ...
  ########################## CODEDEPLOY
  app_name                         = local.ecs_destroy_trigger_flag == true ? null : "${var.env}-${local.ecs_service_name}-${var.cd_app_name}"
  compute_platform                 = var.compute_platform
  cd_app_tags                      = merge(var.cd_app_tags, { Owner = var.owner, Environment = var.env })
  deployment_group_name            = local.ecs_destroy_trigger_flag == true ? null : "${var.env}-${local.ecs_service_name}-${var.cd_group_name}"
  cd_group_tags                    = merge(var.cd_group_tags, { Owner = var.owner, Environment = var.env })
  deployment_config_name           = var.deployment_config_name
  termination_wait_time_in_minutes = var.termination_wait_time_in_minutes

  ########################## CODEBUILD
  codebuild_name                   = local.ecs_destroy_trigger_flag == true ? null : "${var.env}-${local.ecs_service_name}-${var.cb_name}"
  ...
  ########################## IAM
  role_name                        = local.ecs_destroy_trigger_flag == true ? null : [format("%s-role", "${var.cb_name}"), format("%s-pipeline-role", var.cpl_name), format("%s-codedeploy-role", local.deployment_group_name)]
